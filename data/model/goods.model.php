...
class goodsModel extends Model{
	...
	//通过goods_id查询该产品在该店铺下的销量
	public function getGoodVolume($goods_id,$agent_id){
		$model = Model();
		$data = $model->table('order,order_goods')->join('left join')->on('order_goods.order_id=order.order_id')->where("goods_id =".$goods_id." and agent_id = ".$agent_id." and order_state >= '20'")->sum('goods_num');
		return intval($data);
	}
	
	public function getGoodVolumeAll($goods_id){
		$model = Model();
		$data = $model->table('order,order_goods')->join('left join')->on('order_goods.order_id=order.order_id')->where("goods_id =".$goods_id." and order_state >= '20'")->sum('goods_num');
		return intval($data);
	}
