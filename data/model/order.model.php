...
class orderModel extends Model {
	...
	public function getAgentName($agent_id){
		 ...
		 $data = $this->table('agent')->where("agent_id = ".$agent_id)->find();
		 return $data;
	}
    /**
     * 取单条订单信息
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'payment_time desc,order_id desc', $limit = '', $extend = array()){
        ...
        //追加代理商信息
        if (in_array('member',$extend)) {
            $member_id_agent_array = array();
            foreach ($order_list as $value) {
            	if (!in_array($value['agent_id'],$member_id_agent_array)) $member_id_agent_array[] = $value['agent_id'];
            }
            $member_agent_list = Model()->table('member')->where(array('member_id'=>array('in',$member_id_agent_array)))->limit($pagesize)->key('member_id')->select();
            foreach ($order_list as $order_id => $order) {
                $order_list[$order_id]['extend_agent_member'] = $member_agent_list[$order['agent_id']];
            }
        }
        ...
        if (in_array('order_goods',$extend)) {
            ...
            foreach ($order_goods_list as $value) {
                ...
                //print_r($value);die();
                //添加规格代码 zhangyating
               $norms = $this->get_spec_name($value['goods_id']);
               $value['norms'] = $norms;
    ...
    public function get_spec_name($goods_id){
    	$goods_commonid = $this->table('goods')->field('goods_commonid')->where(array('goods_id'=>$goods_id))->find();
    	//只显示一个 
...
//////        }
	...
	public function info($condition){
		//得到条件语句
		$member_info = $this->table('member')->where($condition)->find();
        if (empty($member_info)) {
            return array();
        }

        //追加返回订单扩展表信息
            $member_info['member_id'] = $this->getOrderInfo(array('agent_id'=>$member_info['member_id']));
	}
