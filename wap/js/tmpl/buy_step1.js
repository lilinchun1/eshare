	...
	$.ajax({//提交订单信息
		...
		success:function(result){
			...
			$.each(data.store_cart_list,function(k,v){//循环店铺
				...
				i++;
				htmldata+='<p class="buys-yt-tlt">店铺名称：'+data.agent_info.agent_name+'</p>';
						$.each(v.goods_list,function(k1,v1){//循环商品列表
							...
							htmldata+='<div class="clearfix">'
												+'<a class="img-wp" href="'+WapSiteUrl+'/index.php?act=product&goods_id='+v1.goods_id+'">'
													+'<img src="'+v1.goods_image_url+'"/>'
												+'</a>'
												+'<div class="buys1-pdlcnt">'
													+'<p><a class="buys1-pdlc-name" href="'+WapSiteUrl+'/index.php?act=product&goods_id='+v1.goods_id+'">'+v1.goods_name+'</a></p>'
						...
						htmldata+= '<div class="shop-total"><span id="store'+k+'" style="display:none"></span><p>快递包邮</p>';
    ...
    $('#buy_step2').click(function(){//提交订单step2
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		if(!result.datas.pay_sn || result.datas.pay_sn.pay_sn != ''){
        			//location.href = WapSiteUrl+'/tmpl/member/order_list.html';
					location.href = WapSiteUrl+'/tmpl/member/order_new.php';
        		}
        		return false;
        	}
        });
