$(function(){
	...
	function initPage(page,curpage){
		$.ajax({
			type:'post',
			url:ApiUrl+"/index.php?act=member_order&op=order_list_detail&page="+page+"&curpage="+curpage+"&order_id="+$("input[name=order_id]").val(),	
			data:{key:key},
			dataType:'json',
...
function pay_(url,key,sn){
	...
	if(type=='1'){
		window.location.href=url+"/index.php?act=member_payment&op=pay&key="+key+"&pay_sn="+sn+">";	
	}
	if(type=='2'){
		alert("暂未开通");
	}
