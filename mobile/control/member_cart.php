...
class member_cartControl extends mobileMemberControl {
    ...
    public function cart_listOp() {
		...
		//2014-8-29 购物车显示当前agent_id下的产品
        $condition = array('buyer_id' => $this->member_info['member_id'],'agent_id' => $agent_id);
        ...
        output_data(array('agent_info' => $this->agent_info,'cart_list' => $cart_list,'agent_id' => $agent_id, 'sum' => ncPriceFormat($sum)));
    }
    ...
    public function cart_addOp() {
		if(!(intval($_SESSION['agent_id']))){
			
			output_error('经销商获取参数错误');
		...
		//qinwei v1.2.1 2014-12-16 修改BUG 同一商品无法多次加入购物车
		$init = $model_cart->getCartInfo(array('goods_id'=>$goods_info['goods_id'], 'buyer_id' => $this->member_info['member_id']),'cart_id,goods_id,goods_num');
		if(intval($init['goods_num'])){
			 $quantity = $quantity+intval($init['goods_num']);
			 $condition['buyer_id'] = $this->member_info['member_id'];
             $condition['cart_id']  = $init['cart_id'];
		...
		if(intval($_SESSION['agent_id'])){
			$param['is_agent'] = 1;
			$param['agent_id'] = $_SESSION['agent_id'];
		}
