...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
		...
		//2014-8-25 qw 根据agent_id 获取store_id
		...
		if(empty($_GET['agent_id']) || intval($_GET['agent_id']) <= 0){
			...
			$err = 0;
			session_start();
			$_SESSION['agent_id']=intval($_GET['agent_id']);
		}
		
		$agent_id = intval($_GET['agent_id']);
		//$model_agent_store = Model('agent_store');
		//$store_id = $model_agent_store->where(array('agent_id'=>$agent_id))->find();
    
		$model_agent = Model('agent');
	
		$agent_info = $model_agent->getAgentInfoByID($agent_id);
		//print_r($agent_info);
       ...
       // }
		...
		//2014-8-25 qw 条件获取
		$condition['store_id'] = $agent_info['store_ids'][0];
		
        //所需字段
        $fieldstr = "goods_id,goods_commonid,store_id,goods_name,goods_price,goods_marketprice,goods_image,goods_salenum,evaluation_good_star,evaluation_count,goods_storage";
        ...
        output_data(array('goods_list' => $goods_list,'err' => $err,'agent_info'=>$agent_info), mobile_page($page_count));
    }
