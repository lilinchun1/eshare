 ...
 * @author zhangyating
 * @return string $url_path 改变头像尺寸的路径
 * @since 海参v1.1.1
 */
function getWxImg($url,$size){
	$flag = strpos($url,"wx.qlogo.cn/mmopen");
	if($flag){
		if(!strripos($url,"/",30)){
			$url_path = $url."/".$size;
		}else{
			$url_path = substr_replace($url,"",strripos($url,"/")+1).$size;
		}
	}else{
		$url_path = $url;
	}
...
function getMemberAvatarForID($id){
	if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/avatar_'.$id.'.jpg')){
		...
		$info  = $model_member->getMemberInfo(array('member_id' => $id));
		if($info['member_avatar']){
			return $info['member_avatar'];
		}else{
		//zhangyating end
...
//秦伟 随机获取分享数据
function getShareInfo($store_id) {
    $model = Model();

	$store_id = intval($store_id);
	$data = $model->table('store_wxshare')->where(array('store_id' => $store_id))->order ('rand()')->find();
	$data['wxs_picture'] = wxShareImage($data['wxs_picture'], $store_id);
	$data['wxs_title'] = str_replace(array("\n","\r"), '', $data['wxs_title']);
	$data['wxs_content'] = str_replace(array("\n","\r"), '', $data['wxs_content']);
	return $data;
}
 ...
 * @return string
 */
function wxShareImage($image_name, $store_id) {
    if ($image_name != '') {
        return UPLOAD_SITE_URL.'/shop/store/'.$store_id.'/'.$image_name;
    }
    return UPLOAD_SITE_URL.'/shop/store/default_share_image.jpg';
}
