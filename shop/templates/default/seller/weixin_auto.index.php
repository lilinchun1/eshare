...
<div class="tabmenu">
  ...
  <a href="javascript:void(0)" class="ncsc-btn ncsc-btn-green" onclick="go('index.php?act=weixin_auto&op=auto_add1');" title="添加回复">添加回复</a> 
  <?php }?>
  </div>
<table class="ncsc-table-style">
  ...
  <tbody>
    ...
    <?php foreach($output['auto_list'] as $key => $value){?>
    <tr class="bd-line">
      <td><?php echo $value['event_type'];?></td>
      <td class="tl"><?php echo $value['content'];?></td>
      <?php if($value['event_type']=='many'){?>
      <td class="nscs-table-handle"><span><a href="<?php echo urlShop('weixin_auto', 'auto_edit',array('keyword_id'=>$value['keyword_id'],'event_type'=>$value['event_type']));?>" class="btn-blue"><i class="icon-edit"></i>
        <p>详细</p></a></span><span><a nctype="btn_del_group" data-group-id="<?php echo $value['keyword_id'];?>" href="javascript:;" class="btn-red"><i class="icon-trash"></i>
...
<form id="del_form" method="post" action="<?php echo urlShop('weixin_auto', 'auto_del');?>">
