<div class="eject_con">
   <form method="post" action="index.php?act=weixin_keyword&op=keyword_saves&keyword_id=<?php echo $_GET['keyword_id']?>" id="recommend_form" enctype="multipart/form-data">
    <input type="hidden" name="type" value="many" />
    <input type="hidden" name="reply_id" value="<?php echo $_GET['reply_id'];?>"/>
    <input type="hidden" name="keyword" value="<?php echo $output['key_info']['keyword']?>"/>
    <input type="hidden" name="old_pic_url" value="<?php echo $output['keyword_info']['pic_url'];?>"/>
    <dl>
      <dt><i class="required">*</i>关键字：</dt>
      <dd>
        <p>
          <?php if(empty($output['key_info']['keyword'])){?>
          <input type="text" class="text w400" name="keyword" value="<?php echo empty($output['keyword_info']['keyword'])?'':$output['keyword_info']['keyword']?>" placeholder="" />
          <?php }else{?>
          <input type="text" class="text w400" name="keyword" value="<?php echo $output['key_info']['keyword']?>" disabled="disabled" placeholder="" />
          <?php }?>
        </p>
      </dd>
    </dl>
    ...
    <dl>
      ...
      <dd>
        <p>
          <input type="text" class="text w100" name="order_num" value="<?php echo empty($output['keyword_info']['order_num'])?'':$output['keyword_info']['order_num']?>" placeholder="" />
        </p>
      </dd>
    </dl>
    <dl>
      ...
      <dd>
        <div class="ncsc-upload-thumb store-sns-pic">
          <p><img nc_type="logo1" width="100" height="100" src="<?php echo wxShareImage($output['keyword_info']['pic_url'],$_SESSION['store_id']);?>"/></p>
        </div>
        <div class="handle">
