...
<div id="footer" class="wrapper">
  <p><a href="<?php echo SHOP_SITE_URL;?>"><?php if($_SESSION['seller_is_admin']==1){ ?><?php echo $lang['nc_index'];?><?php }else{?><?php }?></a>
    <?php if(!empty($output['nav_list']) && is_array($output['nav_list'])){?>
    <?php foreach($output['nav_list'] as $nav){?>
    <?php if($nav['nav_location'] == '2'){?>
   <?php if($_SESSION['seller_is_admin'] ==1){?> | <a  <?php if($nav['nav_new_open']){?>target="_blank" <?php }?>href="<?php switch($nav['nav_type']){
    ...
    }?>"><?php echo $nav['nav_title'];?></a><?php }else {?><?php }?>
    <?php }?>
    <?php }?>
    <?php }?>
  </p>
  Copyright (c) 2013-2014 易享科技 版权所有<br />
