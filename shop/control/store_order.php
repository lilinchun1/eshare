...
class store_orderControl extends BaseSellerControl {
	...
	public function indexOp() {
        ...
        if ($_GET['agent_id'] != '') {
            $condition['agent_id'] = $_GET['agent_id'];
        }
        if ($_GET['agent_name'] != '') {
        ...
        $order_list = $model_order->getOrderList($condition, 20, '*', 'payment_time desc,order_id desc','', array('order_goods','order_common','member'));
        ...
        foreach ($order_list as $key => $order_info) {
        	...
        	$sql_point_num = "SELECT point_num FROM shopnc_order_point WHERE order_id = " . $order_list[$key]['order_id'];
        	$que_point_num = $model_order->query($sql_point_num);
			$order_list[$key]['point_num'] = $que_point_num[0]['point_num']?$que_point_num[0]['point_num']:0;
        }
