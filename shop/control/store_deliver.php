...
class store_deliverControl extends BaseSellerControl {
	...
	public function indexOp() {
		...
		//存入分页
		Tpl::output('show_page',$model_order->showpage());//分页显示输出				
	...
	public function sendOp(){
		...
		if (chksubmit()){
			try {
                ...
                //zhangyating 2014-9-28 添加微信发货通知
                $orderInfo = $model_order->where(array("order_id"=>$order_id))->find();
                $memberInfo = Model('member')->where(array("member_id"=>$orderInfo['buyer_id']))->find();
                
                //判断是否是微信支付
                if($memberInfo['member_wxopenid'] && $orderInfo['payment_code'] == "wxpay"){
                	
                	$s = file_get_contents(MOBILE_SITE_URL."/api/payment/wxpay/NoticeShipment.php?openid=".$memberInfo['member_wxopenid']."&transid=".$orderInfo['trade_no']."&out_trade_no=".$orderInfo['pay_sn']);
                	$returnInfo = json_decode($s);
                	file_put_contents("adviceZhangyating/adviceOfgoods.html", print_r($s,1)."|时间是：".date("Y-m-d h:i:s")."\n",FILE_APPEND);
